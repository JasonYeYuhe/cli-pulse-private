#!/usr/bin/env python3
"""
CLI Pulse v1.31.0 — ASC metadata + submit.

Copy of submit_v1_30_0.py (same proven, idempotent ASC API flow). Only
TARGET_VERSION / TARGET_BUILD / WHATS_NEW_* changed. v1.31.0 supersedes the
in-flight 1.30.0 (build 75, still WAITING_FOR_REVIEW) — the cancel->rename
prep flow turns the editable 1.30.0 version into 1.31.0 and binds build 78.

Because users upgrade 1.29.1 -> 1.31.0 (1.30.0 was never released), the
...
Per platform (iOS + macOS), idempotent / safe to re-run:
  ...
  2. (prep) Find/rename/create an editable 1.31.0 appStoreVersion.
  3. (prep) Wait for build 78 -> processingState=VALID, then bind it.
...
Usage:
  submit_v1_31_0.py prep   [IOS MAC_OS]   # steps 1-5, no submit
  submit_v1_31_0.py submit [IOS MAC_OS]   # step 6 only
  submit_v1_31_0.py all    [IOS MAC_OS]   # 1-6
"""
import jwt
import time
import requests
...
# v1.31.0 What's New per locale. No backticks anywhere — Apple's What's
# New field rejects them per feedback_asc_release_workflow.

WHATS_NEW_EN = """v1.31.0

- Fixed (Mac): no more repeated keychain permission prompt while reading your Claude credentials — grant access once and it stays quiet.
- Quota-bar pace markers show whether you are ahead of or behind your expected usage pace, with warning-threshold ticks.
- New per-provider usage history chart on iPhone and Mac.
- Usage-pace summaries across the app, and a redesigned Apple Watch glance (Pulse / Quota / Live / Alerts).
- Reliability and performance improvements."""

WHATS_NEW_ZH_HANS = """v1.31.0

- Mac 端修复:读取 Claude 凭据时不再反复弹出钥匙串授权窗口——授权一次即可保持安静。
- 配额条新增节奏标记,显示你领先还是落后于预期用量节奏,并带有预警阈值刻度。
- iPhone 与 Mac 新增各服务商用量历史图表。
- 全应用显示用量节奏摘要,并重新设计了 Apple Watch 一览界面(Pulse / Quota / Live / Alerts)。
- 稳定性与性能改进。"""

WHATS_NEW_JA = """v1.31.0

- Mac の修正:Claude の認証情報を読み取る際に繰り返し表示されていたキーチェーンの許可ダイアログを解消しました。一度許可すれば表示されません。
- クォータバーのペースマーカーで、想定ペースに対して進んでいるか遅れているかを警告しきい値の目盛りとともに表示します。
- iPhone と Mac にプロバイダ別の使用履歴グラフを追加。
- アプリ全体で使用ペースの概要を表示し、Apple Watch のグランスを刷新(Pulse / Quota / Live / Alerts)。
- 信頼性とパフォーマンスを改善しました。"""

WHATS_NEW_KO = """v1.31.0

- Mac 수정: Claude 자격 증명을 읽을 때 반복적으로 나타나던 키체인 권한 요청을 없앴습니다. 한 번 허용하면 다시 표시되지 않습니다.
- 할당량 막대의 페이스 마커로 예상 사용 페이스보다 앞서는지 뒤처지는지 경고 임계값 눈금과 함께 표시합니다.
- iPhone과 Mac에 제공업체별 사용 기록 차트를 추가했습니다.
- 앱 전반에 사용 페이스 요약을 표시하고 Apple Watch 글랜스를 새롭게 디자인했습니다(Pulse / Quota / Live / Alerts).
- 안정성과 성능을 개선했습니다."""

WHATS_NEW_ES_ES = """v1.31.0

- Corrección (Mac): se eliminó el aviso de permiso del llavero que aparecía repetidamente al leer tus credenciales de Claude; concede el acceso una vez y dejará de molestar.
- Los marcadores de ritmo en las barras de cuota muestran si vas por delante o por detrás de tu ritmo de uso previsto, con marcas de umbral de aviso.
- Nuevo gráfico de historial de uso por proveedor en iPhone y Mac.
- Resúmenes del ritmo de uso en toda la app y un vistazo del Apple Watch rediseñado (Pulse / Quota / Live / Alerts).
...
# App Review notes — v1.31.0 is reliability work plus inactive groundwork.
REVIEW_NOTES = (
    "This update is reliability work plus inactive groundwork. On Mac it "
    "fixes a repeated keychain permission prompt that could appear when the "
    "app reads Claude credentials (a read cooldown). It also lands internal "
    "plumbing for managed CLI sessions that is disabled by default — no UI "
    "entry point is enabled and behavior is unchanged for users. No new "
    "capabilities, entitlements, or background modes were added. No demo "
...
# ─── 2. Find or create an editable 1.31.0 appStoreVersion per platform ───
...
# ─── 3. Wait for build 78 to be VALID, then bind it ──────────────────────
...
def main():
    ...
    if mode not in ("prep", "submit", "all"):
        print("Usage: submit_v1_31_0.py [prep|submit|all] [IOS MAC_OS]")
        sys.exit(1)
