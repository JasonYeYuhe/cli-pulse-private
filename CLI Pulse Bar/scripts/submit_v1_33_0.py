#!/usr/bin/env python3
"""
CLI Pulse v1.33.0 — ASC metadata + submit (iOS + macOS).

Copy of submit_v1_32_3.py (same proven, idempotent ASC API flow). Only
TARGET_VERSION / TARGET_BUILD / WHATS_NEW_* / REVIEW_NOTES changed.

v1.33.0 ships client-side hardening + a small feature, all in CLIPulseCore
(so both iOS and macOS):
  - Live service-status coverage for more providers (ElevenLabs, Groq, Warp,
    Moonshot, Deepgram, Windsurf, Augment).
  - Reduced background CPU/battery from local usage checks (bounded collector
    fan-out + a throttle on the Claude CLI usage probe).
  - Hardened the realtime session stream: a strict decoder allow-list + size
    ceiling, and a navigation guard on the in-app web view.
No new capabilities, background modes, network entitlements, or data
collection were added. Submit BOTH platforms (IOS + MAC_OS).
...
Per platform, idempotent / safe to re-run:
  ...
  2. (prep) Find/rename/create an editable 1.33.0 appStoreVersion.
  3. (prep) Wait for build 83 -> processingState=VALID, then bind it.
...
Usage:
  submit_v1_33_0.py prep   [IOS MAC_OS]   # steps 1-5, no submit
  submit_v1_33_0.py submit [IOS MAC_OS]   # step 6 only
  submit_v1_33_0.py all    [IOS MAC_OS]   # 1-6
"""
import jwt
import time
import requests
...
# API key: honor an ASC_KEY_PATH override, else the local
# ~/.appstoreconnect/private_keys copy, else the iCloud Downloads copy. The
# iCloud Drive copy is TCC-protected and can read as "Operation not permitted"
# under headless runs, so it must not be the only source.
def _resolve_key_path():
    env = os.environ.get("ASC_KEY_PATH")
    if env:
        return os.path.expanduser(env)
    local = os.path.expanduser("~/.appstoreconnect/private_keys/AuthKey_DMMFP6XTXX.p8")
...
API_KEY_PATH = _resolve_key_path()
APP_ID = "6761163709"
...
# v1.33.0 What's New per locale. No backticks anywhere — Apple's What's
# New field rejects them per feedback_asc_release_workflow.

WHATS_NEW_EN = """v1.33.0

- Live service status now covers more providers, including ElevenLabs, Groq, Warp, Moonshot, Deepgram, Windsurf and Augment.
- Reduced background CPU and battery use from usage checks.
- Hardened the live session stream, with reliability and security improvements throughout."""

WHATS_NEW_ZH_HANS = """v1.33.0

- 服务状态现已覆盖更多服务商,包括 ElevenLabs、Groq、Warp、Moonshot、Deepgram、Windsurf 和 Augment。
- 降低了用量检查带来的后台 CPU 与电量消耗。
- 加固了实时会话数据流,并进行了全面的稳定性与安全性改进。"""

WHATS_NEW_JA = """v1.33.0

- サービスステータスが ElevenLabs、Groq、Warp、Moonshot、Deepgram、Windsurf、Augment など、より多くのプロバイダーに対応しました。
- 使用状況チェックによるバックグラウンドの CPU とバッテリー消費を削減しました。
- ライブセッションのストリームを強化し、信頼性とセキュリティを全体的に改善しました。"""

WHATS_NEW_KO = """v1.33.0

- 서비스 상태가 ElevenLabs, Groq, Warp, Moonshot, Deepgram, Windsurf, Augment 등 더 많은 제공업체를 지원합니다.
- 사용량 확인으로 인한 백그라운드 CPU 및 배터리 사용량을 줄였습니다.
- 실시간 세션 스트림을 강화하고 전반적인 안정성과 보안을 개선했습니다."""

WHATS_NEW_ES_ES = """v1.33.0

- El estado del servicio ahora cubre más proveedores, incluidos ElevenLabs, Groq, Warp, Moonshot, Deepgram, Windsurf y Augment.
- Se redujo el uso de CPU y batería en segundo plano de las comprobaciones de uso.
- Se reforzó la transmisión de sesiones en vivo, con mejoras de fiabilidad y seguridad en general."""
...
# App Review notes — v1.33.0. No capability changes this train.
REVIEW_NOTES = (
    "This is a maintenance and hardening update for iOS and macOS. It adds "
    "live service-status coverage for more providers (ElevenLabs, Groq, Warp, "
    "Moonshot, Deepgram, Windsurf, Augment), reduces background CPU and battery "
    "usage from local usage checks, and hardens the realtime session stream "
    "(strict input validation plus a navigation guard on the in-app web view). "
    "No new capabilities, background modes, network entitlements, or data "
    "collection were added. No demo account or special hardware is required to "
    "review; on Mac the app also works in local mode without an account. "
...
# ─── 2. Find or create an editable 1.33.0 appStoreVersion per platform ───
...
# ─── 3. Wait for build 83 to be VALID, then bind it ──────────────────────
...
def main():
    ...
    if mode not in ("prep", "submit", "all"):
        print("Usage: submit_v1_33_0.py [prep|submit|all] [IOS MAC_OS]")
        sys.exit(1)
