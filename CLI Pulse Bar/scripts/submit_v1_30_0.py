#!/usr/bin/env python3
"""
CLI Pulse v1.30.0 — ASC metadata + submit.

Adapted verbatim from submit_v1_22_0.py (same proven, idempotent ASC API
flow). Only TARGET_VERSION / TARGET_BUILD / WHATS_NEW_* / REVIEW_NOTES
changed. v1.30.0 ships real, user-visible Apple-platform features
(quota-bar pace markers + per-provider usage history chart + Apple Watch
glance redesign) plus macOS main-thread hang fixes — so What's New
honestly describes them (no dark-ship caveat).
...
Per platform (iOS + macOS), idempotent / safe to re-run:
  ...
  2. (prep) Find/rename/create an editable 1.30.0 appStoreVersion.
  3. (prep) Wait for build 75 -> processingState=VALID, then bind it.
...
Usage:
  submit_v1_30_0.py prep   [IOS MAC_OS]   # steps 1-5, no submit
  submit_v1_30_0.py submit [IOS MAC_OS]   # step 6 only
  submit_v1_30_0.py all    [IOS MAC_OS]   # 1-6
"""
import jwt
import time
import requests
...
# v1.30.0 What's New per locale. No backticks anywhere — Apple's What's
# New field rejects them per feedback_asc_release_workflow.

WHATS_NEW_EN = """v1.30.0 — see your usage pace at a glance.
...
WHATS_NEW_ZH_HANS = """v1.30.0 — 一眼看清你的用量节奏
...
WHATS_NEW_JA = """v1.30.0 — 使用ペースをひと目で。
...
WHATS_NEW_KO = """v1.30.0 — 사용 페이스를 한눈에.
...
WHATS_NEW_ES_ES = """v1.30.0 — tu ritmo de uso de un vistazo.
...
# App Review notes — v1.30.0 adds no new capabilities/entitlements.
REVIEW_NOTES = (
    ...
    "fixes. No new capabilities, entitlements, or background modes were "
    "added. No demo account or special hardware is required to review; on "
    "Mac the app also works in local mode without an account. Externally "
    "it behaves like the approved prior version with these additive "
    "features. Thank you for the review."
)
...
# ─── 2. Find or create an editable 1.30.0 appStoreVersion per platform ───
...
# ─── 3. Wait for build 75 to be VALID, then bind it ──────────────────────
...
def main():
    ...
    if mode not in ("prep", "submit", "all"):
        print("Usage: submit_v1_30_0.py [prep|submit|all] [IOS MAC_OS]")
        sys.exit(1)
