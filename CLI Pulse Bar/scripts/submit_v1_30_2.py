#!/usr/bin/env python3
"""
CLI Pulse v1.30.2 — ASC metadata + submit.

Copy of submit_v1_30_0.py (same proven, idempotent ASC API flow). Only
TARGET_VERSION / TARGET_BUILD / WHATS_NEW_* changed. v1.30.2 supersedes the
in-flight 1.30.0 (build 75, still WAITING_FOR_REVIEW) — the cancel->rename
prep flow turns the editable 1.30.0 version into 1.30.2 and binds build 76.

Because users upgrade 1.29.1 -> 1.30.2 (1.30.0 was never released), the
...
Per platform (iOS + macOS), idempotent / safe to re-run:
  ...
  2. (prep) Find/rename/create an editable 1.30.2 appStoreVersion.
...
Usage:
  submit_v1_30_2.py prep   [IOS MAC_OS]   # steps 1-5, no submit
  submit_v1_30_2.py submit [IOS MAC_OS]   # step 6 only
  submit_v1_30_2.py all    [IOS MAC_OS]   # 1-6
"""
import jwt
import time
import requests
...
# v1.30.2 What's New per locale. No backticks anywhere — Apple's What's
# New field rejects them per feedback_asc_release_workflow.

WHATS_NEW_EN = """v1.30.2 — more reliable Mac companion CLI.

- Fixed: the companion CLI now shows as installed and running right after you install it on Mac, and is reliably re-checked when you reopen the menu.
- Fixed: Claude usage is detected immediately after you grant folder access — no relaunch needed.
- Also in 1.30: quota-bar pace markers with warning-threshold ticks, a per-provider usage history chart on iPhone and Mac, usage-pace summaries, and a redesigned Apple Watch glance (Pulse / Quota / Live / Alerts).
- Further macOS reliability and responsiveness improvements."""

WHATS_NEW_ZH_HANS = """v1.30.2 — 一眼看清你的用量节奏
...
WHATS_NEW_JA = """v1.30.2 — 使用ペースをひと目で。
...
WHATS_NEW_KO = """v1.30.2 — 사용 페이스를 한눈에.
...
WHATS_NEW_ES_ES = """v1.30.2 — tu ritmo de uso de un vistazo.
...
# App Review notes — v1.30.2 adds no new capabilities/entitlements.
REVIEW_NOTES = (
...
# ─── 2. Find or create an editable 1.30.2 appStoreVersion per platform ───
...
def main():
    ...
    if mode not in ("prep", "submit", "all"):
        print("Usage: submit_v1_30_2.py [prep|submit|all] [IOS MAC_OS]")
        sys.exit(1)
