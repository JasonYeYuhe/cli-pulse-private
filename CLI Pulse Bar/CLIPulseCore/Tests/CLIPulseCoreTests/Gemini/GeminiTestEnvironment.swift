// Vendored verbatim from steipete/CodexBar
// Tests/CodexBarTests/GeminiTestEnvironment.swift (MIT, © Peter
// Steinberger) — temp-dir Gemini CLI / credential scaffolding for the
// probe ports. `#if os(macOS)`-wrapped (fake-binary symlinks + POSIX
// 0o755 bash scripts are macOS test scaffolding; the test target only
// runs on macOS). See GeminiStatusProbe.swift for the full MIT notice.
