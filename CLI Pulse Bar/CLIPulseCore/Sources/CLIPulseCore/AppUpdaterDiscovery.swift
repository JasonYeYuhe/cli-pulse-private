...
extension AppUpdater {
    ...
    ///
    /// Gates on the live task handles, NOT on `state == .checking`: the
    /// initial `state` value IS `.checking` with no request behind it, so a
    /// state check would return early on every fresh launch and passive
    /// discovery would never run at all — the exact bug this method exists
    /// to fix (codex review).
    @MainActor
    public func refreshIfStale(now: Date = Date(),
                               maxAge: TimeInterval = 24 * 60 * 60) async {
        if refreshTask != nil || downloading { return }
